...
class Open3DApp:
    ...
    def _on_close_confirm(self):
        ...
        # Prevent multi-threaded tasks generated by sub threads from being executed before exiting.
        force = True
        try:
    ...
    def start(self):
        ...
        threading.Thread(target=continuously_update_gui, daemon=True).start()
        threading.Thread(target=self._run_smartqsm, daemon=True).start()
    ...
    def _run_smartqsm(self):
        ...
        for i, (cloud_path, file_size) in enumerate(self._path_to_file_size.items()):
            ...
            try:
                ...
                # Density adaptation for skeletonization
                scale = 1.0
                if config.get("using_density_adaptation_in_skeletonization", False):
                    kdtree: KDTree = KDTree(points)
                    scale = config.get("voxel_down_size", 0.01) / kdtree.query(points, k=2)[0][:, 1].mean()
                    self._current_status = "Extracting skeleton with density adaptation enabled..."
                else:
                ...
                generator = skeletonization.run(points=points * scale)
                ...
                skeletal_points = skeletal_points / scale
                radii = radii / scale
    ...
    for i, (cloud_path, file_size) in enumerate(path_to_file_size.items()):
        ...
        try:
            ...
            if config.get("using_density_adaptation_in_skeletonization", False):
                kdtree: KDTree = KDTree(points)
                scale = config.get("voxel_down_size", 0.01) / kdtree.query(points, k=2)[0][:, 1].mean() 
                print("Extracting skeleton with density adaptation enabled...")
            else:
